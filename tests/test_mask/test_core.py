...
def test_plot_axes_match_array_shape() -> None:
    """plot() axes reflect the mask's native pixel dimensions."""
    mask = Mask._from_array(np.zeros((10, 20), dtype=np.bool_))
    fig = mask.plot()
    assert fig.layout.xaxis.range[1] == 20
...
def test_remove_noise_cleans_single_pixel_noise() -> None:
    ...
    assert mask.array[0:10, :].sum() == 200
