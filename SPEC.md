...
- **Saving**: `frame.save(path) → Frame` writes the array to disk as a tiled TIFF with EXIF metadata and returns a new `Frame` pointing to `path`. No Celestack metadata is embedded. If `frame.is_tiled` returns `True`, the frame is copied verbatim instead of re-encoded.
...
- **Timestamp**: a `float | None` property. The getter returns (in priority order): an explicitly set override, a float derived from EXIF datetime, or `None`. The setter allows external code (e.g. Project) to assign pseudo-timestamps. `save_downscaled` requires a non-`None` timestamp and raises `ValueError` if it is unset. On proxy frames, the timestamp is embedded in the Celestack metadata by `save_downscaled`, making proxies self-describing. For full-res frames, timestamps come from EXIF; pseudo-timestamps for full-res frames without EXIF are set via the setter before `save_downscaled` is called.
...
- **Two kinds of files**: Frame transparently handles external files (user-provided TIFF/JPEG/PNG with EXIF metadata, downscale factor defaults to 1) and Celestack proxy files (tiled TIFFs with embedded Celestack metadata — fully self-describing). Full-res files written by Celestack are tiled TIFFs with EXIF but no Celestack metadata, and are treated the same as external files by the Frame class.
...
- **Ingest note**: all Celestack-written files use tiled TIFF layout, enabling efficient tile reading. Proxy files additionally embed Celestack metadata and are fully self-describing.
