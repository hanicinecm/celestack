...
- **Lazy array access**: `frame.array` loads the image into memory on first access and caches it. `frame.load()` loads it explicitly. `frame.unload()` drops the cached array to free memory.
...
- **Subtraction**: `frame.subtract(other, *, inplace=False)`. When `inplace=False` (default), returns a new detached `Frame` with the pixel-wise saturating difference, inheriting metadata and timestamp from `self`. When `inplace=True`, modifies `self` in place, detaches it from its backing path, and returns `None`. Both operands must match in shape, dtype, bit depth, and downscale factor.
